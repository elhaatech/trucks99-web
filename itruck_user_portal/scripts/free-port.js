/**
 * Free a TCP port before starting Next.js (fixes EADDRINUSE).
 * Usage: node scripts/free-port.js 3002
 *
...
function freePortWindows(p) {
  ...
  // 1) PowerShell Get-NetTCPConnection (most reliable on modern Windows)
  const psCandidates = [
    path.join(
  ...
  // 2) Fallback: netstat.exe with full path
  if (pids.size === 0) {
