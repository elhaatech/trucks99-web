...
function isPublicProductPayload(payload: Record<string, unknown>): boolean {
  if (hasMarketplaceBearerToken()) return false;
  return (
...
function bitRecordHttpClient(payload: Record<string, unknown>) {
  if (hasMarketplaceBearerToken()) return api;
...
function productMarketplaceClient(payload: Record<string, unknown>) {
  return bitRecordHttpClient(payload);
}
...
export async function listProductOffers(params: {
  ...
  const client = bitRecordHttpClient(listBody);
