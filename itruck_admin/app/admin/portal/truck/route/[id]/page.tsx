...
import { TruckRoutePage } from "../../_components/router/TruckRoutePage";

export default function TruckRouteRoutePage() {
  return <TruckRoutePage />;
}
...
import TruckRoutesTable from "@/app/trucks/components/TruckRoutesTable";
...
function TruckRoutePageDuplicate() {
  ...
  if (!id) return <Box sx={{ p: 2 }}>Invalid truck id.</Box>;
      ...
      <PageHeader
        ...
        subtitle={
          truck?.registrationNumber
            ? `Routes for ${truck.registrationNumber}`
            : "Manage from/to routes and price for this truck."
        }
        action={
          <Button variant="outlined" onClick={() => router.push(appRoutes.truck.list())}>
            Back to truck list
          </Button>
        }
      />
