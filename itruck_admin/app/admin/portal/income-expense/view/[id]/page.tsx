...
export default function IncomeExpenseViewPage() {
  const router = useRouter();
  const params = useParams();
  const id = typeof params?.id === "string" ? params.id : "";
  const hasValidId = useInvalidIdRedirect(id, routes.incomeExpense.list());
  ...
  if (loading) {
    return (
      <ModulePageLayout
        ...
        subtitle="Loading entry details…"
        breadcrumbs={[
          { label: "Dashboard", href: routes.dashboard() },
          { label: "Income & Expense", href: routes.incomeExpense.list() },
          { label: "Details" },
        ]}
        backButton={<BackButton fallback={routes.incomeExpense.list()} />}
        showAds={false}
      >
        <ViewPageSection title="Entry Information">
          <ViewDetailGrid>
  ...
  const categoryName =
    item.category && typeof item.category === "object"
      ? (item.category as { categoryName?: string }).categoryName
      : null;
  const statusColor = item.status?.toLowerCase() === "active" ? "success" : "default";

  return (
    <ModulePageLayout
      title={`${isIncome ? "Income" : "Expense"} — ₹${Number(item.amount ?? 0).toLocaleString("en-IN")}`}
      subtitle="Income or expense entry details"
      breadcrumbs={[
        { label: "Dashboard", href: routes.dashboard() },
        { label: "Income & Expense", href: routes.incomeExpense.list() },
      ...
      action={
        <Button variant="contained" startIcon={<EditOutlinedIcon />} onClick={() => router.push(routes.incomeExpense.edit(id))}>
          Edit
        </Button>
      }
    >
      <ViewPageSection title="Transaction Details" subtitle="Type, amount, and category">
        <ViewDetailGrid>
          <DetailField
            label="Type"
          ...
          <DetailField
            ...
            value={`₹${item.amount != null ? Number(item.amount).toLocaleString("en-IN") : "—"}`}
          />
          <DetailField label="Category" icon={<CategoryOutlinedIcon />} value={categoryName || item.category_id || "—"} />
          <DetailField label="Remarks" icon={<NotesOutlinedIcon />} value={item.remarks || "No remarks"} fullWidth />
          <DetailField
            label="Status"
