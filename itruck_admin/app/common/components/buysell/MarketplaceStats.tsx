...
export function MarketplaceStatsCards({ stats }: MarketplaceStatsProps) {
  ...
  const STAT_CONFIG = [
    { 
      ...
      progress: 100, 
      footer: "100% currently active" 
    },
    { 
      key: "activeListings" as const, 
      label: "ACTIVE LISTINGS", 
      accent: DASHBOARD_ACCENTS.green, 
      progress: (stats.activeListings / total) * 100, 
      footer: `${stats.activeListings} of ${stats.totalListings} total` 
    },
    { 
      key: "soldVehicles" as const, 
      label: "SOLD", 
      accent: DASHBOARD_ACCENTS.purple, 
      progress: (stats.soldVehicles / total) * 100, 
      footer: `${((stats.soldVehicles / total) * 100).toFixed(0)}% of inventory` 
    },
    { 
      key: "totalOffers" as const, 
      label: "OFFERS", 
      accent: DASHBOARD_ACCENTS.amber, 
      progress: stats.totalListings ? (stats.totalOffers / stats.totalListings) * 100 : 0, 
      footer: `~${(stats.totalOffers / total).toFixed(0)} per listing` 
    },
  ];
  ...
  return (
    ...
    >
      {STAT_CONFIG.map(({ key, label, accent, progress, footer }) => (
        <Box
          key={key}
          sx={{
            display: "flex",
        ...
        >
          ...
          <Box sx={{ mt: "auto" }}>
            <Box sx={{ width: "100%", height: 4, bgcolor: accent.bg, borderRadius: 2, mb: 1.5 }}>
              <Box sx={{ width: `${Math.min(progress, 100)}%`, height: "100%", bgcolor: accent.main, borderRadius: 2 }} />
            </Box>
